...
export class TechnicalAnalysis {
    ...
    return candles
      .map((candle) => {
        ...
        if (Array.isArray(candle)) {
          // [time, open, high, low, close, volume]
          if (typeof candle[4] === 'number') return candle[4];
          if (typeof candle[1] === 'number') return candle[1];
          return null;
        }
  ...
  // ===================== MACD =====================
  // افتراضياً: fast=12, slow=26, signal=9
  calculateMACD(candles, fastPeriod = 12, slowPeriod = 26, signalPeriod = 9) {
    ...
    const filteredMacd = macdLine.filter((v) => v != null);
    const signalRaw = this.calculateEMA(filteredMacd, signalPeriod);

    const signalLine = new Array(macdLine.length).fill(null);
    let idx = 0;
    for (let i = 0; i < macdLine.length; i++) {
      if (macdLine[i] == null) continue;
      if (idx < signalRaw.length) {
        signalLine[i] = signalRaw[idx++];
      }
    }
